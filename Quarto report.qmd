...
concepts.
Blood pressure, measured in millimetres of mercury, is the
pressure within the major arterial system of the body. It is usually separated
in two types of pressures, systolic pressure and diastolic pressure. Systolic 
pressure is the maximum blood pressure during the contraction of the ventricles,
while diastolic pressure is the minimum pressure that is detected right before
the next contraction, it is usually written as systolic pressure/diastolic 
pressure, for example 120/80. These two parameters are key for the correct
perfusion of the organs without causing damage, an ideal blood pressure assures
correct blood flow to all the organs. Having a poor blood flow can lead to the 
organ not functioning properly. Poor blood flow to the brain can cause declining
mental status, lethargy, somnolence and it an extreme situation even coma, 
poor renal blood flow can cause renal failure. On the other hand, high blood 
blood pressure can cause organ damage, it can cause a heart attack, stroke, 
hypertensive renal failure and retinopathy (1). Such is the importance of having
an adequate blood pressure.
Weight is another key parameter for being healthy, having an excessive weight 
can lead to developing heart disease, hypertension, type 2 diabetes, breathing
problems and certain cancers (2) . 
...
After having all the measures, the data was processed and later analysed. 
After the analysis was done , a report was made with Quarto and the results of
the project were shared in Github.

The reference values of blood pressure were obtained from the Center for 
Disease Control and Prevention (3) and the reference weight was calculated
using Calculators.org (4). The analysis was done in R and RStudio.
...
# Since we have a small sample size, only 16 observations and we dont know 
...
# This time , to clean the data and remove the date column that was imported
# from Excel and that is wrong and not needed, we will use a different function 
# than the one used when calculating the systolic pressure, the result is the same:
...
third_data %>% 
        ...
        labs(title= "Systolic and diastolic pressure",
            ...
            "On the Y axis we have the systolic and diastolic
             ...
             The vertical lines shows the moment where the diastolic pressure was
             at its highest")+
 theme_bw()+
 theme(axis.text.x= element_text( size= "5"))
```
...
individual that works out 4 times a week and comparing the results with 
the reference values, it was found that both the systolic and diastolic 
pressure were lower than the average parameters of 120 /80 mmHg of a 
healthy individual, however, there were no statistically significant 
differences between the weight and its reference value.
...
3. High Blood Pressure Symptoms, Causes, and Problems \| cdc.gov.
(2023, August 29). Centers for Disease Control and Prevention.
